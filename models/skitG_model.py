...
from models.model_utils import compute_normal, get_patch_in_input
...
class SKITGModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument(
            "--use_cGAN", type=util.str2bool, default=True, help="choice to replace GAN loss by conditional GAN loss"
        )
        parser.add_argument("--lambda_G1_GAN", type=float, default=1.0, help="weight for GAN loss: GAN(G(X))")
        parser.add_argument("--lambda_G1_L1", type=float, default=100.0, help="weight for L1 loss: L1(G(X), X)")
        parser.add_argument(
            "--lambda_G1_lpips", type=float, default=1.0, help="weight for perceptual loss: LPIPS(G(X))"
        )
        parser.add_argument(
            "--use_cGAN_G2",
            type=util.str2bool,
        ...
        parser.add_argument("--lambda_G2_GAN", type=float, default=5.0, help="weight for G2 GAN loss: GAN(G2(X))")
        parser.add_argument("--lambda_G2_L1", type=float, default=10.0, help="weight for G2 L1 loss: L1(G2(X), X)")
        parser.add_argument(
            "--lambda_G2_lpips", type=float, default=10.0, help="weight for G2 perceptual loss: LPIPS(G2(X))"
        )
        parser.add_argument(
            "--lambda_G2_GAN_feat",
            type=float,
        ...
        parser.add_argument("--n_layers_D2", type=int, default=3, help="only used if which_model_netD2==n_layers")
        parser.add_argument(
            "--num_layer_separate",
            type=int,
            default=4,
        ...
        parser.add_argument(
            "--num_D_D2", type=int, default=3, help="num of layer output to use in the multiple discriminators"
        )
        parser.add_argument(
            "--num_D_D1", type=int, default=3, help="num of layer output to use in the multiple discriminators"
        )
        parser.add_argument("--model_phase", type=str, default="train", help="set model to train/eval for visual_names")

        # configure input and output channels
        parser.add_argument("--sketch_nc", type=int, default=1, help="input channel for sketch image")
        parser.add_argument("--image_nc", type=int, default=3, help="input channel for vision image")
        ...
        # configure data
        parser.add_argument(
            "--data_len", type=int, default=100, help="for single image dataset, do this amount of augmentation"
        )
        parser.add_argument("--batch_size_G2", type=int, default=64, help="batch size for G2 model")
        parser.add_argument("--batch_size_G2_val", type=int, default=128, help="batch size for G2 model for validation")
        parser.add_argument(
            "--center_w",
            type=int,
            default=1280,
        ...
        parser.add_argument(
            ...
            type=int,
            default=1,
            help="multipliction factor from visual resolution to tactile resolution, either 2 or 4",
        )
        ...
        parser.add_argument("--resampling_w_min", type=int, default=1, help="minimum weight for resampling")
        parser.add_argument("--resampling_w_max", type=int, default=10, help="maximum weight for resampling")
        ...
        # configure cross-model, train one model for multiple objects, conditioned on style code
        parser.add_argument(
            "--use_style_code", type=util.str2bool, default=False, help="option to use style code for multiple pants"
        )
        parser.add_argument(
            "--style_code_mode",
            type=str,
            default="concat",
            choices=["concat", "adain"],
            help="ways to integrate style code to latent embedding",
        )
        parser.add_argument(
            "--style_code_mapping_mode",
            type=str,
            default="tile",
            choices=["tile", "project"],
            help="ways to integrate style code vector to spatial latent embedding",
        )
        parser.add_argument("--style_code_dim", type=int, default=512, help="dimension of style code")
        parser.add_argument(
            "--num_layer_style_code", type=int, default=1, help="number of layers to insert the style code"
        )
        # option to edit the dataset
        parser.add_argument(
            "--material_list", nargs="+", default=[], help="list of materials to be used in the dataset"
        )
        # option to use external test sketch or style image for style_code
        # if set the use_external_test_input to False (default), we will just follow the material_list to generate the test dataset
        parser.add_argument(
            "--use_external_test_input", type=util.str2bool, default=False, help="option to use external test sketch"
        )
        parser.add_argument(
            "--test_sketch_material", type=str, default="BlackJeans", help="specify path to the external test sketch"
        )
        parser.add_argument(
            "--test_style_material",
            type=str,
            default="BlackJeans",
            help="specify path to the external style image for style_code",
        )

        # note: set_defaults does not work with "store_true" or "store_false"q action
        parser.set_defaults(
            model="skitG",
            dataset_mode="skit",
            ...
            crop_size=1536,
            no_flip=True,
            dataroot=f"./datasets/singleskit_BluePants_padded_1800_x1/",
            use_style_code=True,
        )
        ...
        if is_train:
            parser.set_defaults(
                ...
                n_epochs_decay=400,  # For lr_policy='linear', linearly decay the rate to zero over the next <opt.n_epochs_decay> epochs.
    ...
    def __init__(self, opt):
        ...
        # TODO: check how to combine the condition here
        ...
        if self.isTrain:
            ...
            if opt.lambda_G1_GAN > 0.0:
                self.model_names.append("D")  # only add discriminator if GAN loss is used
        ...
        if hasattr(opt, "use_diffaug") and opt.use_diffaug and not self.test_edit_S:
    ...
    def optimize_parameters(self, epoch=0, timing=False):
        ...
        if "G" in self.model_names:
            ...
            loss_G2 = self.compute_G2_loss(
                self.train_real_T_concat, fake_T_concat, S_concat, real_I_concat, fake_I_concat, pred_real_T=pred_real_T
            )
    ...
    def set_input(self, input, phase="train", timing=False, verbose=False):
        ...
        self.real_S = input["S"].to(self.device, non_blocking=True)  # equivalent to self.real_A
        ...
        if "I" in input.keys():
            self.real_I = input["I"].to(self.device, non_blocking=True)  # equivalent to self.real_B
        ...
        if hasattr(self.opt, "use_style_code") and self.opt.use_style_code:
            # take in tensors from dataset
            if "style_I" in input.keys():
                style_I = input["style_I"].to(self.device, non_blocking=True)
                if self.opt.use_bg_mask:
                    style_M = input["style_M"].to(self.device, non_blocking=True)
                    style_I = style_I * style_M
                self.style_I = style_I
            else:
                self.style_I = self.real_I
            if torch.is_tensor(self.style_I):
                # convert tensor to PIL image (shape [1, 3, 1536, 1536])
                transform = transforms.ToPILImage()
                self.style_I = [transform(I.cpu()) for I in self.style_I]
            # use clip preprocessing, which takes in a list of PIL image
            self.style_I = torch.cat(
                [self.preprocess_clip(style_I).unsqueeze(0) for style_I in self.style_I], dim=0
            ).to(
                self.device
            )  # [1, 3, 224, 224]
        ...
        if hasattr(self.opt, "use_positional_encoding") and self.opt.use_positional_encoding:
            ...
            # print("create positional encoding for sketch", self.opt.positional_encoding_mode, self.opt.positional_encoding_dim, self.real_S.shape)
    ...
    def compute_evaluation_metric(self, fake_T_concat=None, real_T_concat=None, verbose=False, timing=False, prefix=""):
        """
        Compute evaluation metrics for the current batch of data

        Args:
            fake_T_concat (tensor): the predicted tactile image
            real_T_concat (tensor): the ground truth tactile image
            verbose (bool): option to print out the log info
            timing (bool): option to print out the time for computing the evaluation metrics
            prefix (str): define what phase the data belong to
        ...
        """
        if verbose:
            print(f"model names: {self.model_names}, eval metrics: {self.eval_metrics}, prefix: {prefix}")
        if timing:
            start_time = time.time()

        # Compute metrics for visual output (compare real_I and fake_I)
        real_I = self.real_I.detach()  # range [-1, 1]
        fake_I = self.fake_I.detach()

        if "I_LPIPS" in self.eval_metrics:
            lpips_I = self.eval_LPIPS(real_I, fake_I).cpu().detach().numpy()  # shape (1, 1, 1, 1)
            setattr(
                self, "metric_%sI_LPIPS" % (prefix), np.squeeze(lpips_I)[()]
        ...
        if "I_SIFID" in self.eval_metrics:
            sifid_I = calculate_sifid_given_arrays(real_I, fake_I, normalize=False, device=fake_I.get_device())
            assert len(sifid_I) == 1, "sifid_I should be a list of length 1 for single image"
            setattr(self, "metric_%sI_SIFID" % (prefix), sifid_I[0])

        if "I_PSNR" in self.eval_metrics:
            setattr(self, "metric_%sI_PSNR" % (prefix), PSNR(real_I, fake_I, data_range=2).cpu().detach().numpy())

        if "I_SSIM" in self.eval_metrics:
            setattr(self, "metric_%sI_SSIM" % (prefix), SSIM(real_I, fake_I, data_range=2).cpu().detach().numpy())

        if timing:
            print("eval metric for visual outpput takes time ", time.time() - start_time)
            start_time = time.time()

        # Compute metrics for tactile output (compare real_T_concat and fake_T_concat)
        fake_T_concat = fake_T_concat.detach()
        real_T_concat = real_T_concat.detach()

        # save sample torch tensor to file
        if hasattr(self.opt, "save_T_concat_tensor") and self.opt.save_T_concat_tensor:
            print(f"save T_concat_tensor for current epoch {self.opt.epoch}")
            T_concat = {"real_T_concat": real_T_concat, "fake_T_concat": fake_T_concat}
            tensor_path = os.path.join(
                self.opt.checkpoints_dir, self.opt.name, f"T_concat_{self.opt.epoch}.pth"
        ...
        if "T_SIFID" in self.eval_metrics:
            batch_size = 1
            dims = 64
            resize_size = 299
            ...
            gx_sifid = calculate_sifid_given_arrays(
                T_gx_grp1, T_gx_grp2, batch_size=batch_size, dims=dims, device=self.device
            )
            gy_sifid = calculate_sifid_given_arrays(
                T_gy_grp1, T_gy_grp2, batch_size=batch_size, dims=dims, device=self.device
            )
            T_sifid = np.mean((np.array(gx_sifid) + np.array(gy_sifid)) / 2)
            setattr(self, "metric_%sT_SIFID" % (prefix), T_sifid)

        if "T_LPIPS" in self.eval_metrics:
            resize_size = 224
            real_T_concat_resize = F.interpolate(real_T_concat, (resize_size, resize_size))
            fake_T_concat_resize = F.interpolate(fake_T_concat, (resize_size, resize_size))
            lpips = compute_touch_lpips_loss(self.eval_LPIPS, real_T_concat_resize, fake_T_concat_resize)
            T_lpips = lpips.detach().cpu().numpy()
            setattr(self, "metric_%sT_LPIPS" % (prefix), T_lpips)

        if "T_FID" in self.eval_metrics:
            T_fid = TactilePatchFID(real_T_concat, fake_T_concat)
            setattr(self, "metric_%sT_FID" % (prefix), T_fid)

        if "T_AE" in self.eval_metrics:
            # when compute the actual normal numerically, should set scale_nz as 1
            fake_normals = compute_normal(fake_T_concat, scale_nz=1)
            real_normals = compute_normal(real_T_concat, scale_nz=1)
            T_AE = compute_surface_normal_angle_error(real_normals, fake_normals, mode="evaluate")  # shape [N, 32, 32]
            T_AE = T_AE.mean().cpu().detach().numpy()
            setattr(self, "metric_%sT_AE" % (prefix), T_AE)

        if "T_MSE" in self.eval_metrics:
            MSE = MeanSquaredError()
            T_MSE = torch.mean((real_T_concat - fake_T_concat) ** 2).cpu().detach().numpy()
            setattr(self, "metric_%sT_MSE" % (prefix), T_MSE)
    ...
    def get_current_visuals(self):
        ...
        This function overwrite the one in base_model.py, because for skitG_model, the visuals are divided into two parts:
        ...
        with torch.no_grad():
            if not self.test_edit_S:  # only visualize patches when the ground truth is available
                if self.data_phase == "test":
                    ...
                    real_T_concat_list = [self.train_real_T_concat, self.val_real_T_concat]
                ...
                if self.real_I is not None:  # real S, I, T are available
                    for phase, coords, real_T_concat, I_mask_concat in zip(
                        ...
                        real_I_concat, offset_x, offset_y, cutout_size = get_patch_in_input(
                            self.real_I, coords, return_offset=True
                        )
                        fake_T_concat = get_patch_in_input(
                            self.fake_T, coords, scale_multiplier=self.opt.T_resolution_multiplier
                        )
                        ...
                        if phase == "val" or phase == "test":
                            self.compute_evaluation_metric(fake_T_concat, self.val_real_T_concat)
                        elif phase == "train":
                            self.compute_evaluation_metric(fake_T_concat, self.train_real_T_concat, prefix="train_")
                        else:
                        ...
                        if draw_bounding_box:
                            ...
                            for i in range(len(self.full_T_coords)):
                                ...
                                new_ROI_x, new_ROI_y, new_ROI_h, new_ROI_w = self.full_T_coords[i]
                        ...
                        for (
                            ...
                            if collage_patch_as_array:
                                ...
                                fake_I_patch = torch.squeeze(
                                    F.interpolate(torch.unsqueeze(fake_I_patch, dim=0), (H, W)), dim=0
                                )
                                fake_I_patch_arr = util.tensor2im(fake_I_patch)
                                real_gx_patch_arr = util.tensor2im(
                                    real_gx_patch, convert_gray_imgs=True, colormap=colormap
                                )
                                fake_gx_patch_arr = util.tensor2im(
                                    fake_gx_patch, convert_gray_imgs=True, colormap=colormap
                                )
                                real_gy_patch_arr = util.tensor2im(
                                    real_gy_patch, convert_gray_imgs=True, colormap=colormap
                                )
                                fake_gy_patch_arr = util.tensor2im(
                                    fake_gy_patch, convert_gray_imgs=True, colormap=colormap
                                )
                    ...
                    for (
                        ...
                        for i, S_patch, fake_I_patch, fake_gx_patch, fake_gy_patch, fake_normal_patch, coords_i in zip(
                            range(len(S_concat)),
                            S_concat,
                            fake_I_concat,
                            fake_gx_concat,
                ...
                if (
                    ...
                    for i in range(len(offset_x)):
                        ...
                        fake_S_sample_arr = util.tensor2im(
                            torch.squeeze(F.interpolate(torch.unsqueeze(self.fake_S_samples[i], 0), (H, W)), dim=0)
                        )  # shape (32, 32, 3)
    ...
    def compute_additional_output(self, coords):
        ...
        S_concat = get_patch_in_input(self.real_S, coords)
        return S_concat, fake_T_concat, real_I_concat, fake_I_concat, aug_real_I_concat, aug_fake_I_concat
    ...
    def forward(self, timing=False):
        ...
        if hasattr(self.opt, "use_positional_encoding") and self.opt.use_positional_encoding:
            output = self.netG(torch.cat((self.real_S, self.S_pe), 1), style_code=self.style_code)
        else:
            output = self.netG(self.real_S, style_code=self.style_code)
        ...
        if not self.test_edit_S:
            ...
            if hasattr(self.opt, "use_diffaug") and self.opt.use_diffaug:
                self.aug_real_I = DiffAugment(self.real_I, policy=self.opt.diffaugment)
                self.aug_fake_I = DiffAugment(self.fake_I, policy=self.opt.diffaugment)
            else:
                # no augmentation
                self.aug_real_I = self.real_I
        ...
        if timing:
            print("forward model G2 takes time ", time.time() - start_time)
            start_time = time.time()

    def compute_D1_loss(self, epoch=0):
        """
        Compute loss for the discriminator for visual image.
        Args:
            epoch (int, optional): current epoch. Defaults to 0.

        Returns:
            loss (float): loss for the discriminator for visual image
        """
    ...
    def compute_D2_loss(
        ...
        if self.opt.lambda_G2_GAN > 0.0 and len(real_T_concat) > 0:
            ...
            S_concat = F.interpolate(S_concat, size=(nh, nw), mode="bicubic", align_corners=False, antialias=True)
            fake_I_concat = F.interpolate(
                fake_I_concat, size=(nh, nw), mode="bicubic", align_corners=False, antialias=True
            ).to(self.device)
            real_I_concat = F.interpolate(
                real_I_concat, size=(nh, nw), mode="bicubic", align_corners=False, antialias=True
            ).to(self.device)
            ...
            if self.opt.netD2 == "multiscale":
                self.pred_fake_T_full = self.pred_fake_T_full[-1][-1].detach()  # [1,1,190,190]
            else:
                ...
                if hasattr(self.opt, "use_cGAN_G2") and self.opt.use_cGAN_G2:
                    if self.use_cGAN_G2_S:
                        ...
                        fake_S_samples = F.interpolate(
                            fake_S_samples, size=(nh, nw), mode="bicubic", align_corners=False, antialias=True
                        )
                    ...
                    if self.use_cGAN_G2_I:
                        ...
                        fake_I_samples = F.interpolate(
                            fake_I_samples, size=(nh, nw), mode="bicubic", align_corners=False, antialias=True
                        ).to(self.device)
                        # concat with contact mask
                        fake_I_mask_samples = torch.ones(
                            fake_I_samples.shape[0], 1, fake_I_samples.shape[2], fake_I_samples.shape[3]
                        ).to(self.device)
    ...
    def _compute_touch_lpips_loss(
        self, criterionLPIPS, fake_T_concat, real_T_concat, lambda_lpips=1, separate_xy=False
    ):
        """
        ...
        loss_lpips_gx = criterionLPIPS(
            torch.unsqueeze(fake_T_concat[:, 0, :, :], 1), torch.unsqueeze(real_T_concat[:, 0, :, :], 1)
        )
        loss_lpips_gy = criterionLPIPS(
            torch.unsqueeze(fake_T_concat[:, 1, :, :], 1), torch.unsqueeze(real_T_concat[:, 1, :, :], 1)
        )
    ...
    def compute_G1_loss(self, epoch=0, pred_real_I=None):
        ...
        if self.opt.lambda_G1_GAN > 0.0:
            if hasattr(self.opt, "use_cGAN") and self.opt.use_cGAN:
                # use conditional GAN loss. feed both input sketch and the output fake vision image to the discriminator
    ...
    def compute_G2_loss(self, real_T_concat, fake_T_concat, S_concat, I_concat, pred_real_T=None):
        """
        ...
        if len(real_T_concat) > 0:
            fake_T_concat_org = fake_T_concat.clone().detach()  # save a copy of fake_T_concat for L1 and lpips loss
            ...
            if self.opt.lambda_G2_lpips > 0.0:
                ...
                if self.isTrain:
                    loss_G2_lpips = self._compute_touch_lpips_loss(
                        self.criterionLPIPS_vgg, fake_T_concat, real_T_concat, self.opt.lambda_G2_lpips
                    )
                else:
                    loss_G2_lpips = self._compute_touch_lpips_loss(
                        self.eval_LPIPS, fake_T_concat, real_T_concat, self.opt.lambda_G2_lpips
                    )
            else:
                loss_G2_lpips = 0.0
