...
class SingleImageDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        if opt.input_nc == 1: 
            A_img = Image.open(self.A_paths[0])
            A_img = ImageOps.grayscale(A_img)
        else:
        ...
        if opt.paired_image or opt.lambda_L1 >0.0:
            print("Set opt paired_image to True")
            self.opt.paired_image = True # Require paired images if L1 loss is used
            # Use the same parameters for A and B
            self.zoom_levels_B = self.zoom_levels_A
            self.patch_indices_B = self.patch_indices_A

        else:
            # Apply random operation for B if no paired data are required
    ...
    def __getitem__(self, index):
        ...
        if self.opt.phase == "train":
            ...
            transform_A = get_transform(self.opt, params=param, method=Image.BILINEAR, grayscale=(self.opt.input_nc == 1))
            A = transform_A(A_img)

            if self.opt.paired_image:
                # Apply the same params of transfromation for A and B
