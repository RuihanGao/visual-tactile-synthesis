...
from myutils import create_log_dir_by_date
...
class SingleSkitDataset(BaseDataset):
    ...
    def __init__(self, opt, verbose=False, default_len=1000):
        ...
        assert (
            len(self.S_paths) == 1
        ), "SingleSkitDataset class should be used with one image in sketch S_paths {}".format(self.S_paths)
        if opt.sketch_nc == 1:
        ...
        if not os.path.exists(self.dir_I):
            ...
            assert os.path.exists(self.dir_I) and os.path.exists(
                self.dir_T
            ), "datasets directories are invalid, \n dir_I {} \n dir_T {}".format(self.dir_S, self.dir_I, self.dir_T)
            self.I_paths = sorted(
                make_dataset(self.dir_I, opt.max_dataset_size)
            )  # load images from '/path/to/data/trainI'
            # Set the images
            assert (
                len(self.I_paths) == 1
            ), "SingleSkitDataset class should be used with one image in sketch and visual image, S_paths {}, I_paths {}".format(
                self.S_paths, self.I_paths
            )
            assert opt.image_nc == 3, "Visual image should have RGB 3 channels"
            self.I_img = Image.open(self.I_paths[0]).convert("RGB")

            self.T_paths = make_touch_image_dataset(
                self.dir_T, opt.max_dataset_size
            )  # load tactile images and coordinates from '/path/to/data/trainT'
        ...
        if opt.subdir_valT is not None:
            self.val_T_paths = make_touch_image_dataset(
                self.dir_valT, opt.max_dataset_size
            )  # load tactile images and coordinates from '/path/to/data/valT'
    ...
    def preprocess_data(self, timing=False, verbose=False, separate_val_set=False):
        ...
        print(f"Preprocess data for singleskit_dataset and save them in cache, len {len(self)}...")
        ...
        for index in range(len(self)):
            ...
            if verbose:
                print(
                    f"after make power 2 transformation {S3.size} resize_ratio_w {resize_ratio_w}, resize_ratio_h {resize_ratio_h}"
                )
            if timing:
    ...
    def find_validate_touch_patches_and_coords(
        self,
        T_size,
        T_paths,
        augmentation_params,
        ...
        if verbose:
            print("In function find_validate_touch_patches_and_coords")
        last_time = time.time()
        ...
        for i in range(int(T_size)):
            ...
            if verbose:
                print(f"touch_data_path {i} {touch_data_path}")
            gx, gy, ROI_x, ROI_y, ROI_h, ROI_w, touch_mask, touch_center_mask = touch_data_loader(
                touch_data_path, convert2im=False, return_mask=load_contact_mask
            )
            if verbose:
            ...
            if "padded" in self.opt.dataroot:
                ROI_x, ROI_y, ROI_h, ROI_w = global_padding_find_coords(
                    ROI_x,
                    ROI_y,
                    ROI_h,
                    ROI_w,
                    padded_size=self.padded_size,
                    org_h=self.opt.center_h,
                    org_w=self.opt.center_w,
                )
                if verbose:
                    print(f"After padding ROI_x {ROI_x}, ROI_y {ROI_y}, ROI_h {ROI_h}, ROI_w {ROI_w}")

            new_ROI_x1, new_ROI_y1, new_ROI_h1, new_ROI_w1 = zoom_find_coords(
                ROI_x,
                ROI_y,
                ROI_h,
                ROI_w,
                scale_factor_h=augmentation_params["scale_factor_h"],
                scale_factor_w=augmentation_params["scale_factor_w"],
            )
            if verbose:
                print(
                    f"After zoom_find_coords new_ROI_x {new_ROI_x1}, new_ROI_y {new_ROI_y1}, new_ROI_h {new_ROI_h1}, new_ROI_w {new_ROI_w1}"
                )

            valid_patch, new_ROI_x2, new_ROI_y2, new_ROI_h2, new_ROI_w2 = crop_find_coords(
                new_ROI_x1,
                new_ROI_y1,
                new_ROI_h1,
                new_ROI_w1,
                augmentation_params["crop_size_h"],
                augmentation_params["crop_size_w"],
                augmentation_params["resize_ratio"],
                augmentation_params["crop_pos_x"],
                augmentation_params["crop_pos_y"],
            )
            if verbose:
                print(
                    f"After crop_find_coords new_ROI_x {new_ROI_x2}, new_ROI_y {new_ROI_y2}, new_ROI_h {new_ROI_h2}, new_ROI_w {new_ROI_w2}"
                )

            new_ROI_x3, new_ROI_y3, new_ROI_h3, new_ROI_w3 = make_power_2_find_coords(
                new_ROI_x2,
                new_ROI_y2,
                new_ROI_h2,
                new_ROI_w2,
                augmentation_params["resize_ratio_w"],
                augmentation_params["resize_ratio_h"],
            )
            if verbose:
                print(
                    f"After make_power_2_find_coords new_ROI_x {new_ROI_x3}, new_ROI_y {new_ROI_y3}, new_ROI_h {new_ROI_h3}, new_ROI_w {new_ROI_w3}"
                )

            if valid_patch:
                valid_indexes_list.append(i)
                valid_ROI3_list.append(
                    [int(round(new_ROI_x3)), int(round(new_ROI_y3)), int(round(new_ROI_h3)), int(round(new_ROI_w3))]
                )
                valid_ROI1_list.append(
                    [int(round(new_ROI_x1)), int(round(new_ROI_y1)), int(round(new_ROI_h1)), int(round(new_ROI_w1))]
                )
                valid_ROI2_list.append(
                    [int(round(new_ROI_x2)), int(round(new_ROI_y2)), int(round(new_ROI_h2)), int(round(new_ROI_w2))]
                )
        ...
        calc_weight = (
            True if hasattr(self.opt, "w_resampling") and self.opt.w_resampling else False
        )  # option to weight differently for each patch based on edge info
        ...
        (
            ...
            all_I_images,
            all_M_images,
        ) = self.process_all_valid_patches(
            valid_indexes_list,
            valid_ROI3_list,
            valid_ROI2_list,
            valid_ROI1_list,
            ...
            timing=timing,
            verbose=verbose,
            save_compare_vision_touch_plot=save_compare_vision_touch_plot,
            I1=I1,
            I2=I2,
            I3=I3,
            S3=S3,
            M3=M3,
            method=method,
            compute_SIM_patches=compute_SIM_patches,
            S_tf=S_tf,
            I_tf=I_tf,
            M_tf=M_tf,
            is_train=is_train,
        ...
        if verbose:
            print("total_num_valid_patch %d" % total_num_valid_patch)
    ...
    def process_all_valid_patches(
        ...
        for i in range(len(valid_indexes_list)):
            ...
            if load_contact_mask:
                ...
                gx_im, gy_im, ROI_x, ROI_y, ROI_h, ROI_w, touch_mask, touch_center_mask = touch_data_loader(
                    touch_data_path, convert2im=False, return_mask=load_contact_mask
                )  # Load the tactile data, the ROI info is useless (coordinates of the tactile patch in vision iamge is handled by new_ROI)
                assert (
                    touch_mask is not None and touch_center_mask is not None
                ), "Need valid touch mask and touch center mask"
                ...
                for center_x, center_y in zip(center_xs_contact, center_ys_contact):
                    square_mask = touch_mask[
                        center_y - patch_crop_size_t // 2 : center_y + patch_crop_size_t // 2,
                        center_x - patch_crop_size_t // 2 : center_x + patch_crop_size_t // 2,
                    ]
                    ...
                    offset_x = np.round(
                        (new_ROI_x + crop_pos_x_square / resize_ratio_square) * self.opt.T_resolution_multiplier
                    )
                    offset_y = np.round(
                        (new_ROI_y + crop_pos_y_square / resize_ratio_square) * self.opt.T_resolution_multiplier
                    )
                    cutout_size = np.round(
                        augmentation_params["patch_crop_size"] / resize_ratio_square * self.opt.T_resolution_multiplier
                    )
                    ...
                    M_patch = np.array(
                        M3.crop((offset_x, offset_y, offset_x + cutout_size, offset_y + cutout_size))
                    )  #  min 0, max 255
                ...
                for square_idx in selected_square_idxes:
                    center_x, center_y, square_mask = (
                        center_xs[square_idx],
                        center_ys[square_idx],
                    ...
                    crop_pos_y_square = int(crop_pos_y_t / self.opt.T_resolution_multiplier)
                    # convert to tensor
                    gx_tensor = T_tf(gx_im_square)
                    gy_tensor = T_tf(gy_im_square)
                    # save the patch data, T_images,T_coords
                    gxy = torch.cat((gx_tensor, gy_tensor), 0)
                    assert gxy.shape == (2, patch_crop_size_t, patch_crop_size_t), (
                        "gxy shape %s, center_x %d, center_y %d, patch_crop_size_t %d gx_im.shape %s, gy_im.shape %s"
                        % (str(gxy.shape), center_x, center_y, patch_crop_size_t, str(gx_im.shape), str(gy_im.shape))
                    )
                    T_images.append(gxy)
                    T_coords.append(
                        [
                            ...
                            new_ROI_h,
                            new_ROI_w,
                            augmentation_params["patch_crop_size"],
                            resize_ratio_square,
                            crop_pos_x_square,
                            crop_pos_y_square,
                        ]
                ...
                gx_im_square, resize_ratio_square, crop_pos_x_t, crop_pos_y_t = crop_img(
                    gx_im, patch_crop_size_t, patch_crop_size_t, method
                )
                assert resize_ratio_square == 1, "Maker sure that all patches are larger than 32 pixels"
                gy_im_square, _, _, _ = crop_img(
                    gy_im,
                    patch_crop_size_t,
                    patch_crop_size_t,
                    method,
                    resize_ratio=resize_ratio_square,
                    crop_pos_x=crop_pos_x_t,
                    crop_pos_y=crop_pos_y_t,
                )
            ...
            if save_compare_vision_touch_plot:
                ...
                assert (
                    I1 is not None and I2 is not None and I3 is not None
                ), "I1, I2, I3 are required for save_compare_vision_touch_plot"
                logs_date_dir = create_log_dir_by_date()
                ...
                image_patch = I3.crop(
                    (
                        new_ROI_x + crop_pos_x_square,
                        new_ROI_y + crop_pos_y_square,
                        new_ROI_x + crop_pos_x_square + augmentation_params["patch_crop_size"],
                        new_ROI_y + crop_pos_y_square + augmentation_params["patch_crop_size"],
                    )
                )
                ...
                rect_zoom = patches.Rectangle(
                    (new_ROI_x1, new_ROI_y1), new_ROI_w1, new_ROI_h1, linewidth=1, facecolor="none", edgecolor="r"
                )
                axes[0, 0].add_patch(rect_zoom)
                axes[0, 0].set_title("I1")

                axes[0, 1].imshow(I2)
                rect_zoom_cropped = patches.Rectangle(
                    (new_ROI_x2, new_ROI_y2), new_ROI_w2, new_ROI_h2, linewidth=1, facecolor="none", edgecolor="r"
                )
                ...
                axes[0, 2].imshow(I3)
                rect_zoom_cropped = patches.Rectangle(
                    (new_ROI_x3, new_ROI_y3), new_ROI_w3, new_ROI_h3, linewidth=1, facecolor="none", edgecolor="r"
                )
                ...
                axes[1, 1].imshow(image_patch)
                axes[1, 1].set_title("square image_patch")
                ...
                print(
                    "crop_pos_x_t", crop_pos_x_t, "crop_pos_y_t", crop_pos_y_t, "patch_crop_size_t", patch_crop_size_t
                )
                axes[2, 1].add_patch(rect_zoom)
                axes[2, 1].set_title("gx_im")
                axes[2, 2].imshow(gx_im_square)
                axes[2, 2].set_title("square gx_im")
        ...
        for patch_counter, T_coord, gx_im_square, gy_im_square in zip(
            range(len(T_coords)), T_coords, gx_im_squares, gy_im_squares
        ):
            (
                new_ROI_x,
                new_ROI_y,
            ...
            S_patch = np.array(
                S3.crop((offset_x, offset_y, offset_x + cutout_size, offset_y + cutout_size))
            ...
            if compute_SIM_patches:
                ...
                # convert to tensor
                assert (
                    S_tf is not None and I_tf is not None and M_tf is not None
                ), "S_tf, I_tf, M_tf are required for compute_SIM_patches"
            ...
            if save_sample_patches:
                ...
                print(
                    f"check S_patch {type(S_patch)}, gx_im_square {type(gx_im_square)}, gy_im_square {type(gy_im_square)}"
                )
                print(
                    f"check S_patch {S_patch.size}, gx_im_square {gx_im_square.shape}, gy_im_square {gy_im_square.shape}"
                )
                ...
                cv2.imwrite(
                    os.path.join(logs_date_dir, f"gx_im_square_{patch_counter}.png"),
                    ((gx_im_square + 1) / 2 * 255).astype(np.uint8),
                )
                cv2.imwrite(
                    os.path.join(logs_date_dir, f"gy_im_square_{patch_counter}.png"),
                    ((gy_im_square + 1) / 2 * 255).astype(np.uint8),
                )
                if I_patch is not None:
                    cv2.imwrite(os.path.join(logs_date_dir, f"I_patch_{patch_counter}.png"), I_patch)
                if M_patch is not None:
                    cv2.imwrite(os.path.join(logs_date_dir, f"M_patch_{patch_counter}.png"), M_patch)
                ...
                for ax, img, title in zip(
                    axes,
                    [S_patch, I_patch, M_patch, gx_im_square, gy_im_square],
                    ["S_patch", "I_patch", "M_patch", "gx_im_sq", "gy_im_sq"],
                ):
                    ax.imshow(img)
                    ax.set_title(title)
                    ax.axis("off")
        ...
        if compute_SIM_patches:
            assert (
                len(S_images) == len(I_images) == len(M_images) and len(S_images) > 1
            ), "S_images, I_images, M_images should have the same length and > 1"
