...
class PatchSkitDataset(SingleSkitDataset):
    ...
    def preprocess_data(self, timing=False, verbose=False, separate_val_set=False):
        ...
        # Preprocess the data. Given the fix amount of valid tactile patches (GelSight pad data), we control the length of dataset by opt.sample_bbox_per_patch
        if timing:
            last_time = time.time()
        ...
        if "zoom" in self.opt.preprocess:
            ...
            if verbose:
                print("after zoom transformation", I1.size)
        else:
            S1 = S_img
            I1 = I_img
            M1 = M_img
        ...
        if self.return_patch:
            ...
            self.data_dict = {
                "S_images": S_images,
                "name": [name for _ in range(len(S_images))],
                "S_paths": [self.S_paths[0] for _ in range(len(S_images))],
                "augmentation_params": [augmentation_params for _ in range(len(S_images))],
            }
            if I_img is not None:
            ...
            if I_img is not None:
                self.data_dict.update(
                    {
                        "I": torch.unsqueeze(I_tensor, 0),
                        "T_images": torch.unsqueeze(T_images, 0),
                        "T_coords": [T_coords],
                        "full_T_coords": [full_T_coords],
                        "I_masks": torch.unsqueeze(torch.unsqueeze(I_masks, 0), -2),
                    }
                )  # TODO: check I_masks shape, whether we need unsqueeze (-2)
            if M_img is not None:
                self.data_dict.update({"M": torch.unsqueeze(M_tensor, 0)})
