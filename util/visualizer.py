    ...
    for label, im_data in visuals.items():
        ...
        if save_raw_arr_vis:
            ...
            if "gx" in label or "gy" in label:
                ...
                )  # datatype float, range (-1,1), shape (C, H, W)
...
class Visualizer:
    ...
    def print_current_losses(self, epoch, iters, losses, t_comp, t_data, t_input):
        ...
        """
        message = (
            "(epoch: %d, iters: %d, computational time: %.3f, data loading time: %.3f, setting input time: %.3f ) \n"
    ...
    def save_current_metrics(
        self,
        eval_metrics,
        return_web_dir=False,
        save_metrics=True,
        epoch=None,
        verbose=False,
        save_metric_index=False,
        i=0,
    ):
        if epoch is None:
            epoch = self.opt.epoch
        web_dir = os.path.join(
            self.opt.results_dir, self.opt.name, "{}_{}".format(self.opt.phase, epoch)
        ...
        if save_metrics:
            ...
            if verbose:
                print("save eval metrics to %s" % dict_path)
        if return_web_dir:
            return web_dir
    ...
    def plot_epoch_time(self, epoch, epoch_time):
        ...
        if self.use_wandb:
            self.wandb_run.log(
                {"time taken": epoch_time, "epoch": epoch}
            )  # can plot wandb chart with epoch as x-axis label
