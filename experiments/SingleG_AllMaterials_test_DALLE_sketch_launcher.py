...
Sample commands: python -m experiments baseline launch all
python -m experiments baseline launch_test all
...
class Launcher(TmuxLauncher):
    ...
    def common_options(self):
        ...
        dataset = 'data14'
        train_materials = ["BlackJeans", "BluePants", "BlueSports", "BrownVest", "ColorPants", "ColorSweater", "DenimShirt", "FlowerJeans",  "FlowerShorts", "GrayPants", "GreenShirt", "GreenSkirt", "GreenSweater", "GreenTee", "NavyHoodie", "PinkShorts", "PurplePants", "RedShirt", "WhiteTshirt", "WhiteVest"] 
    ...
    def test_commands(self):
        ...
        for opt in self.common_options():
            ...
            if any('best' in s for s in os.listdir(checkpoint_dir)):
                ...
                assert any('400' in s for s in os.listdir(checkpoint_dir)), f"no suitable checkpoint exists for {train_material} model"
                epoch = 400 # use 400 epoch. some are trained to 400, some are trained to 500. take the minimum
            opt.set(epoch=epoch)
            # opt.set(gpu_ids=-1)
            option_list.append("python test.py " + str(opt.set(eval="", preprocess='none', data_len=1, num_touch_patch_for_logging=100, batch_size_G2=100, save_raw_arr_vis=True)))
